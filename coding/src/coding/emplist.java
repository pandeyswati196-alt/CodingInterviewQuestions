...
public class emplist {
	...
	public static void main(String[] args) {
		...
		Optional<Employee> op= employeeList.stream().collect(Collectors.maxBy(Comparator.comparingDouble(Employee::getSalary)));
		Employee opp=op.get();
		System.out.println(opp.getName());
		//get name of emp joining after year 2015
		//yeh code se pura details mil gaya sirf naam chaiye toh map karo
		//employeeList.stream().filter(x->x.getYearOfJoining()>2015).collect(Collectors.toList()).forEach(System.out::println);
		...
		//what is avg salary of each department.
		...
		//youngest male emp in product dev department
		Optional<Employee> empp=employeeList.stream().filter(emp->emp.getDepartment()=="Product Development"&&emp.getGender()=="Male").min(Comparator.comparingInt(Employee::getAge));
		
	DoubleSummaryStatistics sum=employeeList.stream().collect(Collectors.summarizingDouble(Employee::getSalary));
	List<Employee> emp=employeeList.stream().sorted((e1,e2)->e1.getName().compareTo(e2.getName())).collect(Collectors.toList());
